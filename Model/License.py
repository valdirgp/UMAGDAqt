...
class License:
    ...
    # Cria o arquivo de solicitação de licença
    def create_lincense_request(self, username, instname, temp_window: QWidget):
        try:
            # Seleciona diretório para salvar o arquivo
            save_file_path = QFileDialog.getExistingDirectory(temp_window, "Selecione o diretório para salvar a licença")

            if save_file_path:
                file_path = f'{save_file_path}/request_license_UMAGDA.utc'
                with open(file_path, 'w+', encoding="utf-8") as file:
                    file.write(f'Instituição: {instname}\nNome: {username}\nID: {self.fisical_address}')

                QMessageBox.information(temp_window,
                                        self.util.dict_language[self.lang]["mgbox_success"],
                                        self.util.dict_language[self.lang]["mgbox_success_reqlc"])
                temp_window.close()

        except Exception as error:
            QMessageBox.warning(temp_window,
                                self.util.dict_language[self.lang]["mgbox_error"],
                                self.util.dict_language[self.lang]["mgbox_error_reqlc"])

    # Codifica o endereço físico (MAC)
    ...
    # Verifica se o endereço físico está licenciado
    def verify_fisical_adress(self):
        try:
            with open('license_UMAGDA.utc', 'r') as file:
                file_text = file.read()
                encoded_address = self.encode_address()
                return bool(re.search(encoded_address, file_text))
        except Exception:
            return False
