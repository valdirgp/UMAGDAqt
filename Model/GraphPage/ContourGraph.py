...
class ContourGraph(GraphsModule):
    ...
    def plot_contour(self, local_downloaded, stations, selected_types, bold_text, 
                     grid_graph, start, end, selected_dates, data_with_stations):
            """
            ...
            for plot_type in slct_types:
                ...
                if Z is None or np.all(np.isnan(Z)):
                    QMessageBox.information(
                        None,
                        self.util.dict_language[self.lang]["mgbox_error"],
                        f"{self.util.dict_language[self.lang]['mgbox_interpolacao']} {plot_type} {self.util.dict_language[self.lang]['mgbox_interpolacao2']}"
                    )
                    self.can_plot = False
                    return
