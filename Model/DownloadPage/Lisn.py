...
class Lisn(DownloadModule):
    def __init__(self, language, root=None):
        super().__init__(language, root)
        self.util = Util()
    # creates a list with all stations from the LISN network

    def obter_coordenadas_estacoes_lisn(self):
        """
        Lê todas as estações LISN a partir do arquivo readme_stations.txt ou do readme oficial.
        """
        estacoes = {}
        ...
        #url = 'https://github.com/Thiago-Mancilla/LISN_STATIONS/blob/main/readme_magnetometer.txt'
        ...
        try:
            ...
            print(f"Erro ao obter estações LISN da URL: {e}")
            return {}
    ...
    def create_stationlist(self, ano):
        ...
        codigos = []
        try:
            estacoes = self.obter_coordenadas_estacoes_lisn()
            ...
            QMessageBox.information(
                ...
                f'Erro ao conectar-se com LISN: {e}'
            )
            return [], []

    
    # gets information and control save data
    def initialize_download_Lisn(self, root, local_download, selected_stations, duration, duration_type, start_date):
        self.root = root
    ...
    def add_request_queue(self, station, dt, requests):
        ...
        url = f'https://lisn.igp.gob.pe/database/base/magnetometer/IGP/{st.lower()}/{dt.year}/{dt.month:02}/minute/{st.lower()}_{dt.year % 100}{dt.month:02}{dt.day:02}.min.gz'
        requests.append({'url': url, 'station': station, 'date': dt})
        return requests

    # gets all the text from the lisn file
    responses = []
    responses.append("Lisn")
    def get_file(self):
            ...
            try:
                with self.semaphore_threading:
                    ...
                    if self.is_string_readable_by_line(lines) and len(lines) > 1:
                        ...
                        # file creation
                        directory_path = self.create_dict_path(request_item['station'], str(request_item['date'].year), 'LISN')
