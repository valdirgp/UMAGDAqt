...
import queue
from PyQt5.QtWidgets import QMessageBox  # substitui o tkinter
...
class Embrace(DownloadModule):
    ...
    def create_stationlist(self):
        try:
            ...
            msg.setText("Erro ao conectar-se com embrace")
            msg.exec_()
            return []
    ...
    def initialize_download_Embrace(self, root, local_download, selected_stations, duration, duration_type, start_date):
        ...
        for station in selected_stations:
            ...
            while date != final_date:
                requests = self.add_request_queue(station, date, requests)
        ...
        while not self.request_queue.empty():
            ...
            try:
                with self.semaphore_threading:
                    ...
                    with self.opener.open(request['url'], timeout=20) as file: 
                        raw_data = file.read()
                    ...
                    if self.is_string_readable_by_line(lines) and len(lines) > 1:
                        ...
                        while True:
                            try:
                                if can_count:
                                    ...
                                    if len(line) == 10:
                                        if f'{hh:02}' != line[3]:
                                            if f'{hh-1:02}' == line[3] and f'{mm:02}' == line[4]:
                                                lines.pop(count)
                                                self.insert_clean_data(lines, count, request['date'], hh, mm)
                                            elif f'{line[3]}:{line[4]}' in self.hours_counted:
                                                lines.pop(count)
                                                continue
                                            else:
                                                self.insert_clean_data(lines, count, request['date'], hh, mm)
                                    ...
                                    elif (len(line) > 0 and len(line) < 10) or len(line) > 10:
                                        lines.pop(count)
                                        self.insert_clean_data(lines, count, request['date'], hh, mm)
                                    elif len(line) == 0:
                                        self.insert_clean_data(lines, count, request['date'], hh, mm)
                        ...
                        # file creation
                        directory_path = self.create_dict_path(request['station'], str(request['date'].year), 'EMBRACE')
                        file_path = os.path.join(directory_path, f'{request["station"].lower()}{str(request["date"].year)}{request["date"].month:02}{request["date"].day:02}min.min')
                        with open(file_path, 'w+', encoding='utf-8') as file:
                            for _, line  in enumerate(lines):
                                file.write(line+'\n')
                    else:
                        print(f'Erro no download (não é possível ler arquivo): {request["station"].lower()}{str(request["date"].year)}{request["date"].month:02}{request["date"].day:02}min.min')
            except Exception as error:
                print(f'Erro no download: {request["station"].lower()}{str(request["date"].year)}{request["date"].month:02}{request["date"].day:02}min.min: {error}')
            finally:
